...
def main():
    ...
    parser.add_argument("--train-task", help="Optional real task ID from the train split")
    args = parser.parse_args()
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    manifest = {"domain": "telecom", "splits": {}}
    for split in ("train", "test"):
        tasks = list(get_tasks("telecom", task_split_name=split))
        ...
        rows = [
            {
                ...
                "extra_info": {"index": i, "task_id": str(task.id), "split": split},
            }
